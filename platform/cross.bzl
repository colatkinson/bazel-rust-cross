# Cribbed heavily from https://github.com/duarten/rust-bazel-cross
load("@rules_rust//rust:rust.bzl", "rust_binary")

def _impl(settings, attr):
    _ignore = (settings, attr)
    return {"//command_line_option:platforms": "//platform:i386_bare_metal"}

_i386_bare_metal_platform_transition = transition(
...
_transition_rule = rule(
    ...
    attrs = {
        ...
        "actual_binary": attr.label(cfg = _i386_bare_metal_platform_transition),
...
def i386_bare_metal_rust_binary(name, src_binary):
    _transition_rule(
        name = name,
        actual_binary = src_binary,
    )
